...
// Возвращаем 0, так как этот метод будет переопределяться для каждого типа тренировки.
...
func (t Training) TrainingInfo() InfoMessage {
	...
	speed := t.meanSpeed()
	calories := t.Calories()
	return InfoMessage{t.TrainingType, t.Duration, distance, speed, calories}
}
...
func (w Walking) Calories() float64 {
	...
	return (CaloriesWeightMultiplier*w.Weight + (math.Pow(speed, 2)/heightInMeters)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	speed := s.meanSpeed()
	calories := s.Calories()
	return InfoMessage{s.TrainingType, s.Duration, distance, speed, calories}
}
